...
MP_UPDATE_MAP = re.compile(r"Beatmap: https://osu.ppy.sh/b/(?P<map_id>\d+) (?P<map_repl>.*)")
MP_CHANGED_MAP = re.compile(r"Beatmap changed to: (?P<map_repl>.*) \(https://osu.ppy.sh/b/(?P<map_id>\d+)\)")
MP_CHANGED_MAP2 = re.compile(r"Changed beatmap to https://osu.ppy.sh/b/(?P<map_id>\d+) (?P<map_repl>.*)")
MP_UPDATE_SET = re.compile(r"Team mode: (?P<team_mode>\S+), Win condition: (?P<score_mode>\S+)")
MP_UPDATE_PC = re.compile(r"Players: (?P<player_count>\d+)")
MP_SLOT_INFO = re.compile(r"Slot (?P<slot>\d{1,2})  (?P<status>Ready|Not Ready|No Map)\s*https://osu\.ppy\.sh/u/(?P<user_id>\d+) (?P<user_name>\S*)(?P<flags>.*)")
...
MP_CHANGED_MODE = re.compile(r"Changed match mode to (?P<game_mode>OsuMania|Osu|Taiko|CatchTheBeat)")
MP_CHANGED_MODS = re.compile(r"(Disabled all mods|Enabled (?P<enabled_mods>.*)), (?P<freemod>dis|en)abled FreeMod")
...
MP_JOIN = re.compile(r"(?P<user>.*) joined in slot (?P<slot>\d+)( for team (?P<team>blue|red))\.")
MP_LEFT = re.compile(r"(?P<user>.*) left the game.")
MP_ALL_READY = re.compile(r"All players are ready")
MP_FINISED_PLAYING = re.compile(r"(?P<user>.*) finished playing \(Score: (?P<score>\d+), (?P<status>FAILED|PASSED)\)\.")
